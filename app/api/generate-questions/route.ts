...
export async function POST(req: NextRequest) {
  try {
    ...
    const verticalGuidance: Record<string, string> = {
      ...
      'General':
        'This vertical is often chosen by candidates whose major or background does not map to a named industry (humanities, area studies, liberal arts, career-changers), so treat a non-obvious background as a strength to surface, not a gap to paper over. Do not fall back on lowest-common-denominator behavioral prompts for the whole set: read the specific role title and organization, and make at least two questions target competencies the named role actually requires (infer them — e.g. a "Research Coordinator" implies gathering, verifying, synthesizing, and communicating information). Include at least one question that explicitly invites the candidate to translate a transferable strength — research, writing, a second language, cross-cultural fluency, or making sense of dense or ambiguous material — into value for this specific role. Keep the standard behavioral coverage, but frame it in terms the candidate’s actual background can speak to.',
    }
